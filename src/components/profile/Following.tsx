        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <div ref={popUp} className="bg-[#262626] rounded-lg shadow-lg max-w-sm w-full">
                <div className=" relative m-auto border-b border-gray-200 py-2 px-6">
                    <h2 className="text-xl font-medium text-white text-center">Đang Follow</h2>
                ...
                <div className="h-[310px] mt-4 space-y-4 px-6 overflow-y-scroll">
                    {followingList.length > 0 ? (
                        followingList.map((following) => {
                            return (
                                <Link href={`http://localhost:3000/user/${following.userName}`}>
                        ...
                        <div className="flex items-center justify-center h-full">
                            <p className="text-white/50">{isMyProfile ? "Bạn" : currentUsername} chưa Follow ai</p>
                        </div>
                    )}
                </div>
            </div>
