  ...
  return (
    <div className="absolute left-0 top-full z-50 invisible opacity-0 group-hover/profile:visible group-hover/profile:opacity-100 transition-all duration-300">
      <div className="flex flex-col max-w-[360px] w-[90vw] bg-white rounded-xl shadow-lg mt-2">
        ...
        <div className="p-4 whitespace-normal text-sm !leading-5 font-normal">
...
const Post = ({ post, onVote }: PostProps) => {
  // Debug logging to see what post data is received
  useEffect(() => {
    console.log("Post component received data:", post);
  }, [post]);

  // State cho việc vote
  const [vote, setVote] = useState<number | null>(null);

  // Cập nhật state local khi post data thay đổi
  useEffect(() => {
    console.log("Post userVote value:", post.userVote);
    // Nếu có dữ liệu vote từ server, cập nhật state local
    if (post.userVote !== undefined) {
      setVote(post.userVote);
    }
  }, [post.userVote]);

  // Debug khi vote thay đổi
  useEffect(() => {
    console.log(`Vote state changed for post ${post.id}: ${vote}`);
  }, [vote, post.id]);
  ...
  const handleDownVote = async () => {
    // Gọi hàm xử lý từ prop nếu có (trước khi thay đổi UI local)
    if (onVote) {
      onVote(1);
    }

    // Chỉ cập nhật UI local nếu không có dữ liệu vote từ server
    if (post.userVote === undefined) {
      const newVote = vote === 1 ? null : 1;
      setVote(newVote);
    }
  };
  ...
  return (
    <div className="border-t border-[#212121]">
      <div className="pt-1 pb-3 px-4 mt-1 w-full max-w-[732px] bg-transparent hover:bg-[#f7f9fa] rounded-xl">
        ...
        <div className="text-sm text-black" dangerouslySetInnerHTML={{ __html: post.content || "" }} />
