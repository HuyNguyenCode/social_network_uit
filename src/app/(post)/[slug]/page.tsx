...
const InputComment = () => {
    ...
    return (
        <div className="border border-[#FFFFFF33] rounded-2xl comment-input mt-5">
            {!isFormat ? (<textarea className="w-full h-[84px] p-2 rounded-lg bg-transparent text-white" />) : (<TextEditor editorData={content} setEditorData={setContent} />)}
            <div className="flex flex-row justify-between mt-5 py-1 px-2">
                <button onClick={() => setIsFormat(!isFormat)} className="w-8 aspect-square rounded-full hover:bg-[#283039] text-[#7F93AB] flex items-center justify-center">
                ...
                <div className="flex flex-row gap-2 text-xs font-semibold">
                    <button className="bg-[#1F262E] text-white rounded-full px-[10px]">Cancel</button>
                    <button className="bg-[#464D51] text-white rounded-full px-[10px]">Comment</button>
                </div>
            </div>
        </div>
    )
...
const PostMenu = ({ isOpen, onClose, onEdit, onDelete }: {
    isOpen: boolean;
    onClose: () => void;
    onEdit: () => void;
    onDelete: () => void;
    ...
    return (
        <div className="absolute right-0 top-10 bg-[#1F262E] rounded-xl shadow-lg z-50">
            <div className="py-1 text-[#DBE4E9]">
                <button
                    onClick={onEdit}
                    className="w-[243px] h-10 px-4 py-1 text-left hover:text-white flex items-center justify-start gap-2"
                >
                ...
                <button
                    onClick={onDelete}
                    className="w-full px-4 py-2 text-left hover:text-white flex items-center gap-2"
                >
...
const DeleteConfirmation = ({ isOpen, onClose, onConfirm }: {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-[#1F262E] rounded-xl p-6 w-[400px]">
                <h2 className="text-white text-xl font-semibold mb-4">Delete Post?</h2>
                <p className="text-[#B8C5C9] mb-6">Once you delete this post, it can’t be restored.</p>
                <div className="flex justify-end gap-3">
                    <button 
                        onClick={onClose}
                        className="px-4 py-2 text-white hover:bg-[#283039] rounded-full"
                    >
                        Go back
                    </button>
                    <button 
...
const Page = () => {
    ...
    return (
        <div className="flex flex-col max-w-[732px]">
            <div className="pt-2 pb-0 px-2 my-2 w-full max-w-[732px] bg-transparent rounded-xl">
                <div className="flex flex-row justify-between items-center mb-4 relative">
                    <div className="flex flex-row items-center gap-1 text-[#8BA2AD] text-xs font-semibold">
                        <div className="flex flex-row items-center gap-2 text-white cursor-pointer">
                            <svg className="bg-[#1F262E] rounded-full p-2 hover:bg-[#223237] ease-in-out duration-100" fill="currentColor" height="32" icon-name="back-outline" viewBox="0 0 20 20" width="32" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <p className="hover:text-[#90A9FD]">John Doe</p>
                        </div>
                        <span>•</span>
                    ...
                    <div ref={menuRef} className="relative">
                        <button
                            ...
                            className="hover:bg-[#223237] flex flex-row items-center justify-center rounded-full w-8 h-8 active:bg-[#FFFFFF26]"
                        >
                ...
                {isEdit ? (<div className="border border-[#FFFFFF33] rounded-2xl h-40">
                    <TextEditor editorData={content} setEditorData={setContent} />
                </div>) : (<div className="text-sm text-[#B8C5C9]">
